...
export function AniyaLogo({ size = 24, state = "idle", className = "" }: Props) {
  return (
    <div
      className={`relative shrink-0 ${state === "idle" ? "animate-float" : ""} ${className}`}
      style={{ width: size, height: size }}
      title="Aniya-Studio"
    >
      {state === "idle" && <IdleSVG size={size} />}
      {state === "thinking" && <ThinkingSVG size={size} />}
      {state === "done" && <DoneSVG size={size} />}
    </div>
  );
}

/* ─── Idle: blinking + roaming eyes ───────────────────────────────────── */

function IdleSVG({ size }: { size: number }) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      <style>{`
        .a-blink {
          ...
          animation: a-blink 10s ease-in-out infinite;
        }
        @keyframes a-blink {
          0%, 7%, 9%, 28%, 30%, 48%, 50%, 71%, 73%, 91%, 93%, 100% { transform: scale(1, 1); }
          8%, 8.6% { transform: scale(1, 0.12); }
          29%, 29.6% { transform: scale(1, 0.12); }
          49%, 49.6% { transform: scale(1, 0.12); }
          72%, 72.6% { transform: scale(1, 0.12); }
          92%, 92.6% { transform: scale(1, 0.12); }
        }
        .a-eye-pan {
          transform-origin: 8px 9px;
          animation: a-roam 10s ease-in-out infinite;
        }
        @keyframes a-roam {
          0%, 12%, 100% { transform: translate(0px, 0px) scale(1, 1); }
          18%, 22% { transform: translate(-0.85px, 0px) scale(1, 1); }
          38%, 42% { transform: translate(0.85px, 0px) scale(1, 1); }
          54%, 58% { transform: translate(0px, -0.45px) scale(1.12, 0.92); }
          62%, 66% { transform: translate(0px, 0px) scale(1, 1); }
        }
      `}</style>
      <path fillRule="evenodd" d="M0 16h16V0H0v16z M2 2h12v10H2z" />
      <g className="a-eye-pan">
        <g transform="translate(5,9)">
          <g className="a-blink">
            <path d="M4 8h2v2H4z" transform="translate(-5,-9)" />
          </g>
        </g>
        <g transform="translate(11,9)">
          <g className="a-blink">
            <path d="M10 8h2v2h-2z" transform="translate(-11,-9)" />
          </g>
        </g>
      </g>
    </svg>
...
/* ─── Thinking: scanning eyes + loader spinner ────────────────────────── */

function ThinkingSVG({ size }: { size: number }) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      <style>{`
        .t-eye-pan {
          transform-origin: 8px 9px;
          animation: t-look 3.2s ease-in-out infinite;
        }
        @keyframes t-look {
          0%, 10%, 100% { transform: translate(0px, 0px); }
          18%, 32% { transform: translate(-0.75px, 0px); }
          42%, 56% { transform: translate(0.7px, 0px); }
          66%, 82% { transform: translate(-0.3px, 0px); }
          88%, 96% { transform: translate(0.3px, 0px); }
        }
        .t-blink {
          transform-origin: 0px 0px;
          animation: t-blink 4.8s ease-in-out infinite;
        }
        @keyframes t-blink {
          0%, 34%, 37%, 100% { transform: scale(1, 1); }
          35.5%, 36.3% { transform: scale(1, 0.12); }
        }
        .t-loader-gap {
          transform-origin: 8px 8px;
          animation: t-spin 0.95s linear infinite;
        }
        @keyframes t-spin {
  ...
  return (
    ...
    >
      <style>{`
        .d-smile-eyes {
          transform-origin: 8px 9px;
          animation: d-bounce 2.8s ease-in-out infinite;
        }
        @keyframes d-bounce {
          0%, 100% { transform: translate(0px, 0px); }
          42%, 58% { transform: translate(0px, -0.35px); }
        }
        .d-smile-eye {
          fill: none;
          stroke: currentColor;
        ...
        .d-badge {
          ...
          animation: d-pop 2.4s ease-in-out infinite;
        }
        @keyframes d-pop {
          0%, 100% { transform: scale(1); }
          14% { transform: scale(1.14); }
          26% { transform: scale(0.98); }
          40%, 88% { transform: scale(1.05); }
        }
        .d-circle {
          transform-box: fill-box;
          transform-origin: center;
        ...
        .d-check {
          fill: none;
          stroke: var(--editor-bg, #ffffff);
          stroke-width: 2.35;
          stroke-linecap: round;
          stroke-linejoin: round;
          stroke-dasharray: 16;
          animation: d-draw 2.4s ease-in-out infinite;
        }
        @keyframes d-draw {
          0%, 10% { stroke-dashoffset: 16; }
          30%, 82% { stroke-dashoffset: 0; }
        ...
        .d-ring {
          ...
          transform-box: fill-box;
          transform-origin: center;
          animation: d-burst 2.4s ease-out infinite;
        }
        @keyframes d-burst {
          0%, 18% { opacity: 0; transform: scale(0.82); }
          31% { opacity: 0.24; transform: scale(1.08); }
      ...
      <g className="d-smile-eyes">
        <path className="d-smile-eye" d="M4.25 9.25 Q5 8.25 5.75 9.25" />
        <path className="d-smile-eye" d="M10.25 9.25 Q11 8.25 11.75 9.25" />
      </g>

      <g transform="translate(9.2,-1.6) scale(0.35)">
        <circle className="d-ring" cx="12" cy="12" r="11.25" />
        <g className="d-badge">
          <circle className="d-circle" cx="12" cy="12" r="12" fill="currentColor" />
          <path className="d-check" d="M6.65 12.45 L11.1 15.85 L17.2 8.95" />
        </g>
      </g>
    </svg>
  );
